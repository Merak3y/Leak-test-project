...
void setup() {
  nexInit();
  // Serial0 : for communication B/D <-> PC
  init_setup(node);
...
void loop() {
  ...
  // Initialization
  if (flag_timeEnd == 1U || flag_toMain == 1U || flag_failCur == 1U) {
    if (flag_timeEnd == 1U) {
  ...
  if (flag_timer_500ms == 1U && (flag_start_IMM == 1 || flag_start_DRY == 1)) {
    ...
    else if (gl_STATE == SET_TEST_MODE)
    {
      ...
      // After done setting HW
        ...
        if (flag_failCur == 0U) {
          if (cnt_timer > PERIOD_ALL) { cnt_timer = PERIOD_ALL; }
          sendData2HMI_PRGRS(cnt_timer);
          HMI_DISP_Time((millis() - time_cur) * 0.001, "tProgress");
          HMI_DISP_onText("tPRGRS_per", 0, (int)(((float)cnt_timer / (float)PERIOD_ALL) * 100));
        }
      }
    }
  }
...
void set_elecRegulator(ModbusMaster232 l_node_in, uint16_t l_addr_elec_reg_w_in, uint16_t l_ui_data_in, uint16_t l_addr_elec_reg_r_in) {
  ...
  if(flag_start_setRegul == 1U)
  {
    ...
    if (output_regul >= gl_regOut_target) {
      delay(WAIT_TIME_STABILIZATION);

      // re-check pressure > fail, need to add
...
void set_HMI_Disp()
{
  ...
  gl_avgcur_sum = gl_avgcur_sum + HMI_DISP_AvgCur_WF(61);
}

void set_HMI_Disp_Done()
{
  // if fail
  if (int_cal_sum_arr() > 0) {
    flag_failCur = 1U;
  }
  if (flag_failCur == 1U) {
