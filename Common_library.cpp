...
void sendData2HMI_PRGRS(int i_data_in)
{
  ...
  l_percent_progress = (int)(((float)i_data_in/(float)(PERIOD_ALL))*100);
...
void CheckFail()
{
  if(flag_start_IMM == 1U)
  {
    for (int idx = 0U ; idx < NUM_SEVEN_FOLD; idx++)
    {
      if(stat_sf[idx] == ON)
      {
        if(data_sf[idx]> thr_pressure_each_ma)
        {
    ...
    for (int idx = 0U ; idx < NUM_TWELVE_FOLD; idx++)
    {
      if(stat_tf[idx] == ON)
      {
        if(data_tf[idx]> thr_pressure_each_ma)
        {
  ...
  else if(flag_start_DRY == 1U)
  {
    for (int idx = 0U ; idx < NUM_SEVEN_FOLD_DRY; idx++)
    {
      ...
      {
        if(data_sf_dry[idx]> thr_pressure_each_ma)
        {
    ...
    for (int idx = 0U ; idx < NUM_TWELVE_FOLD_DRY; idx++)
    {
      ...
      {
        if(data_tf_dry[idx]> thr_pressure_each_ma)
        {
